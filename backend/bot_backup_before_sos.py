...
async def veteran_menu(message: types.Message, state: FSMContext = None):
    ...
    nav_kb.append([KeyboardButton(text="🆘 Техпідтримка")])
...
async def show_specialist_cabinet(message: types.Message, spec: dict):
    ...
    kb = [
        ...
        [KeyboardButton(text="🌐 Перейти на Портал", web_app=WebAppInfo(url=f"{PORTAL_URL}?v={timestamp}"))],
        [KeyboardButton(text="❌ Видалити профіль"), KeyboardButton(text="🆘 Техпідтримка")]
    ]
...
async def show_ngo_cabinet(message: types.Message, spec: dict):
    ...
    kb = [
        ...
        [KeyboardButton(text="✏️ Редагувати профіль"), KeyboardButton(text="🌐 Перейти на Портал", web_app=WebAppInfo(url=f"{PORTAL_URL}?v={timestamp}"))],
