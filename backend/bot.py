...
async def handle_contact_request(callback: types.CallbackQuery):
    ...
    if spec:
        ...
        await callback.message.answer(
            ...
            "💡 *Підказка:* Наступного разу ви можете налаштувати свої персональні критерії пошуку (стать спеціаліста чи локацію Черкащини) в Особистому кабінеті в боті, або скористатися нашими зручними розширеними фільтрами безпосередньо на порталі 🌐.",
            parse_mode="Markdown"
        )
